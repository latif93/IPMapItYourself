...
def main():
    ...
    with open("final_processed/final_processed_batch03.json", "r") as f:
        for line in f.readlines():
            ...
            router = list(data.keys())[0]
            #print(data[router].get('city'))
            if ipv4:
                ips.append(ipv4)
                ip_to_loc[ipv4]={'city': data[router].get('city'), 'state_region': data[router].get('state_region'), 'country': data[router].get('country')}
    ...
    engine = Engine(EngineType.RIPE, selected_ips, '5b28e74b-fd17-447f-8e97-49935b173249', validation=False, ip_to_loc=ip_to_loc)
    engine.run()
